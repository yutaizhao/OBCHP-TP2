...
#CHECK if I want to execute this scrpt
...
#END CHECK if I want to execute this scrpt
...
#END CHECK if script in good path

for compiler in gcc clang
do
    rm -rf $compiler #delete prev results if there are
    sleep 2 #so that you can see the files has been really deleted
    mkdir $compiler #create reps for new results
    cd $compiler
    mkdir build
    cd build
    ...
    do
        ...
        make CC=$compiler #change flag then compile
        echo "test for $compiler, $flag, n=$n1, r=$r1"#forgot to make this print french, bref
        #execution
        for (( i=0; i < $run; ++i ))
        do
