...
do
    ...
    paste ./base_$n/build/mean_* > "./base_"$n"/"$CC$n"_base_mean_allflags.txt" #sort result for mean
    paste ./base_$n/build/stddev_* > "./base_"$n"/"$CC$n"_base_stddev_allflags.txt" #sort result for stddev
    paste ./base_$n/build/mbps_* > "./base_"$n"/"$CC$n"_base_mbps_allflags.txt" #sort result for mbps (repeated)
    sed 's/([^)]*)//g' "./base_"$n"/"$CC$n"_base_stddev_allflags.txt" > "./base_"$n"/"$CC$n"_base_stddev_PURE_allflags.txt" #sort result for stddev without %
    #chatgpt:sed 's/([^)]*)//g'
    
    echo "Creating files for unroll" #same thing
...
#les commentaires sont en anglais....shorter faster
#la plupart des commentaires sont ajoutés juste avant le rendu du td..... j'espere que jai pas touche les instructions
